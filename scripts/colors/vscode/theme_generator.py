#!/usr/bin/env python3
"""
Generate VSCode theme via local extension with _watch:true.
Writes a full color-theme.json to a local extension directory — no settings.json
injection. VS Code watches the file and hot-reloads on change.
"""
...
def _write_extension_manifest(ext_dir: Path) -> None:
    """Write package.json with _watch:true so VS Code hot-reloads the theme file."""
    manifest = {
...
def _is_dark_theme(colors: dict) -> bool:
    """Determine if the palette is dark based on background luminance."""
    bg = colors.get("background", colors.get("surface", "#080809"))
    _, _, l = _hex_to_hsl(bg)
    return l < 0.5
...
def _write_theme_file(
    ...
    """Atomic-write a complete color-theme.json (tmp + rename)."""
    theme_path.parent.mkdir(parents=True, exist_ok=True)
    theme_type = "dark" if _is_dark_theme(colors) else "light"
    theme = {
...
LEGACY_EXTENSION_DIRS = ["inir.inir-theme-1.0.0"]


def _clean_legacy_extensions(fork_key: str) -> None:
    """Remove old extension directories from previous iNiR versions."""
    import shutil

    ext_base = _get_ext_dir(fork_key)
    for legacy in LEGACY_EXTENSION_DIRS:
        legacy_dir = ext_base / legacy
...
def generate_vscode_theme(colors_json_path, scss_path, settings_path, fork_key=""):
    ...
    # Install extension (only write manifest once to avoid extension host errors)
    ...
    # Remove legacy injection keys from previous iNiR versions
    for key in [
...
THEME_FILE_NAME = "inir-material-color-theme.json"
